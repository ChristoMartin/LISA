...
cat ./$1 | grep -o "CUDA_VISIBLE_DEVICES.*" | sed 's/bin\/train.sh/bin\/evaluate-exported.sh/' | sed -E  's/run-([0-9])/run-\1\/best_checkpoint/'  |sed 's/.log/.result/' |sed -E "s/config\/.+conf/$target_config/" | while read line; do echo "$line-$config_name"; done|xargs -n 16 -P 4 -L 1 -I '{}' bash -c "{}" # |# | while read line; do echo "$line-${$2##*/}"; done | xargs -n 13 -P 4 -L 1  -I '{}' echo '{}' #xargs -n 16 -P 4 -L 1 -I '{}' bash -c "{}"
