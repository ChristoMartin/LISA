...
cat ./$1 | grep -o "singularity exec .*" | sed 's/bin\/train.sh/bin\/evaluate-exported.sh/' | sed 's/\/lustre\/gk77\/k77015\/.Singularity\/imgs\/LISA.simg/\/home\/u00222\/singularity\/images\/LISA.simg/' | sed -E  's/run-([0-9])/run-\1\/best_checkpoint/'  |sed 's/.log/.result/'|  while read line; do echo "-p $2 --gres=gpu:1 --time=1-06:00:00 --mem=24GB" $line ; done | xargs -n 17 -P 4 -L 1  -I '{}' bash -c "srun {}" #xargs -n 16 -P 4 -L 1 -I '{}' bash -c "{}"
