...
cd $PBS_O_WORKDIR
SAVEDIR=.model/la/injection/glove-dep/conll05-crfnbiaffine-dropout
CONF=config/la/injection/glove-dep/conll05-crfnbiaffine.conf
SINGULARITY_IMG=/lustre/gk77/k77015/.Singularity/imgs/LISA.simg

module add cuda9/9.0.176-cuDNN7.1.4 singularity/2.5.1
{
CUDA_VISIBLE_DEVICES=0 singularity exec --nv $SINGULARITY_IMG bin/train.sh $CONF --save_dir $SAVEDIR/run-1 --num_gpus 1 --parser_dropout  &> $SAVEDIR/run-1/train.log
} &
