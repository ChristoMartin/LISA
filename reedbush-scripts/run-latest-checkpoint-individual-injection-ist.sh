...
cat ./$1 | grep -o "singularity exec .*" | sed 's/bin\/train.sh/bin\/evaluate-exported.sh/'  | sed 's/\/lustre\/gk77\/k77015\/.Singularity\/imgs\/LISA.simg/\/home\/u00222\/singularity\/images\/LISA.simg/'| sed 's/.log/.result/' |sed -E "s/config\/.+conf/$target_config/" | while read line; do echo "$line-$config_name"; done| while read line; do echo "-p $3 --gres=gpu:1 --time=1-06:00:00 --mem=24GB" $line ; done | xargs -n 17 -P1 -I '{}' sh -c "sleep 2; echo \"{}\"" |xargs  -n 17 -P 4 -L 1 -I '{}' bash -c "srun {}" # |# | while read line; do echo "$line-${$2##*/}"; done | xargs -n 13 -P 4 -L 1  -I '{}' echo '{}' #xargs -n 16 -P 4 -L 1 -I '{}' bash -c "{}"
