...
cat ./$1 | grep -o "singularity exec .*" |sed "s#\$CONF#$CONF#" | sed "s#\$SAVEDIR#$SAVEDIR#g" | sed "s#\$SINGULARITY_IMG#$SINGULARITY_IMAGE#" | sed "s#\$ADDITIONAL_PARAMETERS#$ADDITIONAL_PARAMETERS#g"  | while read line; do echo "-p $3 --gres=gpu:1 -J $JOB_NAME --cpus-per-task=9 --mem-per-cpu=10240MB " $line ; done |  xargs -n 16 -P 2 -L 1 -I '{}' bash -c "srun {}"
