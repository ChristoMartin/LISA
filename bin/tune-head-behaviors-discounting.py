...
out_dir = os.path.join(args.output_dir, "tune-discounting-head-behavior-{}-".format(args.exp_remark) + datetime_str)
...
params = {
  ...
  'parse_dep_injection': ['discounting'],
  'special_attention_mode': ['my_discounting', 'okazaki_discounting'],
...
def add_to_partition(_partition, _setting_str, _log_str):
    slurm_cmd = 'srun --gres=gpu:1 --partition=%s --mem=%s --time=1-06:00:00' % (_partition, args.cpu_memory)
